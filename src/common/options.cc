...
std::vector<Option> get_rgw_options() {
  return std::vector<Option>({
	...
	Option("rgw_obj_meta_cache_enabled", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
	.set_default(true)
	.set_description("Enable RGW Object metadata cache")
	.set_long_description(
		"The metadata cache holds metadata entries that RGW requires for processing "
		"requests. Metadata entries can be object xattr, and object omap info "
		"If not found in the cache, entries will be fetched from the backing "
		"RADOS store.")
	.add_see_also("rgw_obj_meta_cache_lru_size"),

	Option("rgw_obj_meta_cache_lru_size", Option::TYPE_INT, Option::LEVEL_ADVANCED)
	.set_default(10000)
	.set_description("Max number of items in RGW Object metadata cache.")
	.set_long_description(
	...
	Option("rgw_obj_meta_cache_expiry_interval", Option::TYPE_UINT,
	   Option::LEVEL_ADVANCED)
	.set_default(15_min)
	.set_description("Number of seconds before entries in the cache are "
			 "assumed stale and re-fetched. Zero is never.")
