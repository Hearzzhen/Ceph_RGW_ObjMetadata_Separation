...
void RGWListBucket::execute()
{
  ...
  if (op_ret >= 0) {
	...
	if (!(s->cct->_conf->rgw_enable_tikv)) {
      ...
      if (!next_marker.empty()) {
        ...
        ldpp_dout(this, 20) << "s_max: " << s_max << " and next_marker: " << 
		  next_marker.name << " has been push in saveMax_G." << dendl;
      }
	} else {
	  ldpp_dout(this, 20) << "tikv is enabled! no need the saveMax_G. next_marker = " << next_marker << dendl;
	}
  }
}
...
void RGWCopyObj::execute()
{
  ...
  op_ret = store->copy_obj(obj_ctx,
			   ...
			   &etag,
			   copy_obj_progress_cb, (void *)this, true
    );
...
void RGWCompleteMultipart::execute()
{
  ...
  op_ret = obj_op.write_meta(ofs, accounted_size, attrs, true);
  if (op_ret < 0)
    return;
